...
// The four axes of the JSON summary.
type Axes struct {
...
func NewSummary(user string, since, until time.Time, counts Counts) Summary {
	...
	return Summary{
		...
		Total: counts.Total(),
		Axes: Axes{
			Commits:       Axis{Count: counts.Commits, Percent: percents.Commits},
			MergeRequests: Axis{Count: counts.MergeRequests, Percent: percents.MergeRequests},
			Issues:        Axis{Count: counts.Issues, Percent: percents.Issues},
			CodeReview:    Axis{Count: counts.CodeReview, Percent: percents.CodeReview},
		},
	}
}
